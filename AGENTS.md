...
```
(`.Codex/launch.json` defines the same server on port 8080.)
...
This is the **"Clean" build** — a faithful port of the Codex Design file `Lost Triangle Clean.dc.html` (pure full-canvas animation, no side panels). 7 scenes: Intro · Root Spiral · Triangle Construction · Cube · Reflected into Form · Dihedral Angles · Close (75s). Palette magenta `#FF00CC` / steel blue / gold on near-black; fonts Syne · Cormorant Garamond · Space Grotesk.
