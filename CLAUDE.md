...
- **Portrait vs landscape.** Fixed 16:9 animation bundles have no portrait layout. Two accepted patterns: (a) ship a portrait variant (as `lost-triangle.html` does via `LostTriangleVideoCleanPortrait`, picked by aspect ratio), or (b) **rotate-to-fill**: on portrait viewports, rotate the landscape composition 90° so it fills the tall screen edge-to-edge (as `mathematics-preview.html` does — the YouTube-fullscreen pattern; the viewer turns the phone to watch). Rotate-to-fill requires the slide to live in an **iframe** sized by the rotated box (`width:100dvh; height:100dvw; rotate(90deg)`), so the content gets a landscape viewport; `preview-slide.html?s=<key>` is the minimal host for plain-React slides. Pointer/touch coordinates map through the CSS transform, so drag and transport controls keep working (verified). Beware inline sizes on the rotated element — an inline `height:100%` overrides the stylesheet's rotated box and crops the content. Do **not** add a "rotate your device" blocker — one existed on the preview page and was removed at the owner's request (July 2026); content must render at every scale.
