...
- **Portrait vs landscape.** Fixed 16:9 animation bundles have no portrait layout. Two accepted patterns: (a) ship a portrait variant (as `lost-triangle.html` does via `LostTriangleVideoCleanPortrait`, picked by aspect ratio), or (b) scale-to-fit letterbox, vertically centered, at any aspect (as `mathematics-preview.html` does — every slide's stage scales itself to the viewport and re-fits on resize/orientationchange). Do **not** add a "rotate your device" blocker — one existed on the preview page and was removed at the owner's request (July 2026); content must render at every scale.
