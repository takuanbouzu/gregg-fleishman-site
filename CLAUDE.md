...
├── explore.html               # THE CUBE — main interactive (7 tabs, 1 hidden — see below)
...
**explore.html** — 7 tabs (1 currently hidden from the tab bar), each its own `<script type="module">` or iframe:
...
7. **Vector House** — GLB assembly viewer for `assets/models/vector-house-pod.glb` (added July 2026, same meshopt re-encode pipeline as Rhombi Pod: 96 MB Draco source → 25 MB). This is Gregg's 45°-fold plywood house system tiled to full-house scale — the *same* node-and-panel module vocabulary as Rhombi Pod (`Panels`/`Hex-1`/`Hex-2` + `NODE PARTS/90`/`45`), confirmed against a DXF geometric analysis of the real building (Rhino/DXF layers are literally named `45` for sloped panels and `90` for flat connectors), plus two more groups this export carries: `DECK` and an unlabeled `Layer1`. 98,892 source triangle primitives / 1,741 GLB mesh nodes — rendering each natively (Rhombi Pod's pattern) measured under 1 fps, so **every layer is merged into a single `BufferGeometry` at load** (`BufferGeometryUtils.mergeGeometries`, already vendored), collapsing the model to ~6 draw calls; each part's original Rhino material colour is baked into a per-vertex `color` attribute before merging (`vertexColors: true`) so the merge stays visually faithful to Gregg's wood/blue/lavender palette instead of flattening to one colour per layer. Because merging destroys the individual CAD parts, **explode is category-level** (each layer's single merged mesh pulls away from the model centre as one block along its own bounding-sphere-center direction), not per original part like Rhombi Pod. If a future GLB needs true per-part explode at this poly count, look at instancing or GPU-side offsets instead of one-`Mesh`-per-part. Do not confuse this file with the pre-existing, unrelated `assets/models/vector-house.glb` (4.4 MB, single merged mesh, used only by the background scene in `assets/lost-triangle/vector-house-scene.js` — no separable parts, not this tab).

   **Quantization pitfall (hit and fixed during integration, watch for it on any future re-encode):** this GLB's meshopt re-encode quantized `position`/`normal` to `Int16Array` with `attribute.normalized = true` (`KHR_mesh_quantization`) — normal and expected. But `THREE.BufferGeometry.applyMatrix4()` round-trips every vertex through `getX/setX`, and for a *normalized* attribute, the setter re-quantizes whatever it's given back into that same narrow int16 range. Baking a real-world placement matrix (tens of units of translation) into that geometry silently **clamped every coordinate to roughly ±1**, collapsing the whole model to a tiny fragment near the origin with no error or warning anywhere — Rhombi Pod's GLB never hit this because its positions weren't quantized. The fix (`dequantize()` in the script): before calling `applyMatrix4`, copy `position`/`normal` out via `getX/getY/getZ` (which *read*-dequantize correctly) into a **plain, non-normalized** `Float32Array` attribute. Any future geometry-baking code (recentre, world-space merge, etc.) on a quantized GLB needs this same dequantize-before-transform step, or it will fail exactly this same silent way.

   **Performance ceiling not fully verified on real hardware:** even after merging to 6 draw calls, this session's sandboxed browser only had swiftshader (CPU software rendering, no real GPU) available, which measured well under 1 fps — but that number is dominated by raw vertex count (≈7.6M vertices across the merged, non-indexed layers, mirroring the original v6 script's `toNonIndexed()` approach for safe merge compatibility), not draw-call count, and CPU software rasterization is 20–100× slower than real GPU hardware for vertex-heavy scenes. It has **not** been confirmed smooth on an actual GPU. If it feels heavy on real devices, the next lever is reducing triangle count before the meshopt encode (`gltf-transform weld`/`simplify` on the 96 MB Draco source) rather than anything in the merge/render code, which is already at minimum draw-call count.
