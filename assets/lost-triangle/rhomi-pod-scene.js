// Rhomi Pod — a single rhombic-system panel unit, parsed directly from
// Gregg's Rhino STEP export (assets/models/rhomi-pod.glb, tessellated from
// the original B-rep solid). Vanilla Three.js r160 ESM, no build step —
// matches the renderer-setup convention in CLAUDE.md and the loader
// pattern in vector-house-scene.js.
import * as THREE from 'three';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';

export function mountRhomiPodScene(container) {
  ...
  loadingEl.textContent = 'loading the pod…';
  ...
  loader.load(
    ...
    (xhr) => {
      if (xhr.total) loadingEl.textContent = 'loading the pod… ' + Math.round((xhr.loaded / xhr.total) * 100) + '%';
    },
    (err) => {
      loadingEl.textContent = 'the pod could not be loaded.';
      console.error('[rhomi-pod-scene] load failed:', err);
    }
  );
